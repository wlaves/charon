...
from config import alias_dir, tv_ip, routes
from common import dummy_ip
import json

def register_routes(app):
    @app.get("/")
    def index():
        # Get the first (and only) TV name and IP
        tv_name, tv_ip_addr = next(iter(tv_ip.items()))
...
"""
    ...
    def state():
        ...
        tv_ip_addr = next(iter(tv_ip.values()))
        tv_route = state.get(tv_ip_addr, {}).get("route") if tv_ip_addr in state else None
